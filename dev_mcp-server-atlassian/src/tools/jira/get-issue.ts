...
export const registerGetIssueTool = (server: McpServer) => {
  server.tool(
    ...
    async (params: GetIssueParams, context: any): Promise<McpResponse> => {
      try {
        ...
        if (!config) {
          ...
          // Lấy thông tin issue với cấu hình từ môi trường
          const result = await getIssueHandler(params, envConfig);
        ...
        // Lấy thông tin issue với cấu hình từ context
